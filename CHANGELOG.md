...
- **Settings tab reorganised**: cards are regrouped — left column holds **Profiles** (with Export/Import inside it) and **Hotkey**; right column holds **Repository / Credits** (renamed from *Repository*), **Display**, **Network**, **Co-Op**, and **Input**. The Profiles panel is reworked into one box: a scrollable profile list with the active one checked, then an Export/Import section with a two-tab switch whose height is fixed so toggling tabs no longer shifts the layout. The what-to-include picker is an outlined box, and choosing a bundled preset to import no longer dumps its whole JSON into the paste field — it's used by reference.
