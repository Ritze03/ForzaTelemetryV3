...
### Added
- **Status-bar Backfire & Gearbox indicators**: the bottom status bar now shows Backfire and Automatic Gearbox state next to the connection light, each with its tab icon. Backfire is green when active, red when off. The Gearbox is green when active, red when off, and pastel-amber **Uncalibrated** while it's enabled but hasn't engaged (i.e. before your first manual upshift teaches it the redline).
